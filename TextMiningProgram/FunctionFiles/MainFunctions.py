...
def analyzeJSON(path: str):
    ...
    for filename in data.keys():
        ...
        dic["NUMERO DE FACTURA"] = getAllInvoiceNumbers(currentText)
        ####################################################
...
def exportToExcel(input : dict, outFilePath : str):
    ...
    FIELDS = {"FILENAME":0, "REGISTRO":1, "FechaRegistro":4, "NIF":9, 
    "Email":59, "Telefono":60, "FechasExpedición":5, "ListaPrecios":61,
    "PrecioTotal":58, "BASE":28, "IVACANT":38, "%IVA":33, "NOMBRE":7, "NUMERO DE FACTURA":11}
    row = 0
    ...
    for filename, data in input.items():
        for key in data.keys():
            if type(data[key]) == list:
                toprint = " | ".join([str(el) for el in data[key]])
            else:
                toprint = data[key]
            worksheet.write(row, FIELDS[key], toprint)
        row +=1
    workbook.close()
